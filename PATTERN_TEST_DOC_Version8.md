...
3. [Структура и интерфейс результатов](#структура-и-интерфейс-результатов)
4. [Логика расчёта результатов](#логика-расчёта-результатов)
...
10. [Требования к окружению и запуску](#требования-к-окружению-и-запуску)
...
- Последовательность экранов:
    1. **Введение** — описание целей теста.
    2. **Ввод данных** — форма для ФИО, контактов, согласий.
    3. **Вопросы теста** — пошаговое прохождение с выбором вариантов.
    4. **Результаты теста** — визуализация и интерпретация результатов, возможность скачать PDF/JSON.
- Все ответы и результаты обрабатываются на клиенте или сервере сайта.
...
- Пользователь открывает ссылку в браузере, где отображается подробный анализ, визуализация и можно скачать PDF-отчет или сырой JSON с ответами.
...
- **Кнопка "Скачать ответы (JSON)"** — для выгрузки исходных данных.
