const pageWidth = 595;
export function createCenteredLine(lineWidth = 200, yPosition = 0) {
  return {
    canvas: [
      {
        type: "rect",
...
// 4 Блок — Контакты
export function makeContactsBlock() {
  const content = [];
  // Заголовок
  content.push({
  ...
  content.push({
    text: [
      { text: "По любым возникающим вопросам можно связаться с автором проекта Еленой Семеновой:", style: "secondText" }
    ],
    margin: [0, 0, 0, 10],
  });
  content.push({
...
export function makeRepeatTestBlock() {
  const content = [];
  content.push({
...
export function makeAuthorBlock() {
  const content = [];
  content.push({
...
export function makeDominantPatternsDescriptionsBlock(
  categories,
  patternResults,
  strongThreshold = 75
) {
  ...
  content.push({
    text: [
      "Ссылка на описание других паттернов: ",
      { text: "https://ai4g.ru/patterns.html", link: "https://ai4g.ru/patterns.html", color: "#007BFF" },
    ],
    style: "secondTextLink",
    margin: [0, 16, 0, 0],
  });
